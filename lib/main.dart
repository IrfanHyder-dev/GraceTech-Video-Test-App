import 'package:flutter/material.dart';
import 'package:gracetech_video_player_app/screens/home_screen.dart';
import 'package:gracetech_video_player_app/screens/video_player_screen.dart';
import 'package:provider/provider.dart';
import 'package:get/get.dart';

import 'package:gracetech_video_player_app/provider/video_provider.dart';

void main() {
  runApp(
      //deffinign the provider in the toplevel fuction for using in whole app
      MultiProvider(providers: [
    ChangeNotifierProvider(
      create: (_) => VideoProvider(),
    ),
  ], child: MyApp()));
}

class MyApp extends StatelessWidget {
  const MyApp({super.key});

  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return GetMaterialApp(
      title: 'Flutter Demo',
      debugShowCheckedModeBanner: false,
      theme: ThemeData(
        // This is the theme of your application.
        //
        // Try running your application with "flutter run". You'll see the
        // application has a blue toolbar. Then, without quitting the app, try
        // changing the primarySwatch below to Colors.green and then invoke
        // "hot reload" (press "r" in the console where you ran "flutter run",
        // or simply save your changes to "hot reload" in a Flutter IDE).
        // Notice that the counter didn't reset back to zero; the application
        // is not restarted.
        primarySwatch: Colors.blue,
      ),

      //Defining app routes for navigation
      routes: {
        HomeScreen.route: (context) => HomeScreen(),
        VideoPlayer.route: (context) => VideoPlayer(),
      },
      home: const HomeScreen(),
    );
  }
}
